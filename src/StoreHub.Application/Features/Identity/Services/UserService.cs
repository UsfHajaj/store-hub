...
public sealed class UserService : IUserService
{
    ...
    public async Task<Result> DeleteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        var roles = await _db.UserRoles.Where(ur => ur.UserId == id).ToListAsync(cancellationToken)
            .ConfigureAwait(false);
        _db.UserRoles.RemoveRange(roles);

        var memberships = await _db.StoreMembers.Where(m => m.UserId == id).ToListAsync(cancellationToken)
            .ConfigureAwait(false);
        _db.StoreMembers.RemoveRange(memberships);
    ...
    private async Task<UserDto> MapUserDtoAsync(Guid userId, CancellationToken cancellationToken)
    {
        var user = await _db.Users.AsNoTracking()
            ...
            .Select(u => new
            {
                ...
                u.IsActive,
                u.LastLoginUtc,
                RoleNamesAr = u.UserRoles
                    .Select(ur => ur.Role.NameAr)
                    .OrderBy(n => n)
                    .ToList(),
                RoleNamesEn = u.UserRoles
        ...
        return new UserDto
        {
            ...
            RoleNames = user.RoleNamesAr.Select((ar, i) =>
                    string.IsNullOrWhiteSpace(ar) ? user.RoleNamesEn.ElementAtOrDefault(i) ?? ar : ar)
                .Where(n => !string.IsNullOrWhiteSpace(n))
                .Cast<string>()
                .ToList(),
            RoleNamesEn = user.RoleNamesEn,
